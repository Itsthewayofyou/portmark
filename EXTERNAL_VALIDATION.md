...
| EV-005 | Medium | Approval replay tracking is kept in checkpoint state, not a dedicated immutable store index. | Add durable approval-use records to `RuntimeStore`. | SQLite and Postgres store approval IDs transactionally; replay is rejected even if checkpoint memory is corrupted; shared store contract tests cover rollback and replay. | TM-008 |
