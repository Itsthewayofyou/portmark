...
binds the format version, the task id, the row generation and the key id, so a sealed checkpoint
cannot be moved to another task or another generation, and a changed byte fails authentication.
...
`status`, `generation`, `closed` and owner columns stay plaintext because the store queries them.
"""
...
def _associated_data(task_id: str, generation: int, key_id: str) -> bytes:
    return canonical_json({"format": "portmark.checkpoint.v1", "task_id": task_id, "generation": int(generation), "key_id": key_id})
...
class CheckpointCodec:
    ...
    def seal(self, task_id: str, generation: int, plaintext: str) -> str:
        nonce = secrets.token_bytes(_NONCE_BYTES)
        sealed = self._keys[self.current_key_id].encrypt(
            nonce, plaintext.encode("utf-8"), _associated_data(task_id, generation, self.current_key_id)
        )
    ...
    def open(self, task_id: str, generation: int, stored: str) -> str:
        ...
        try:
            plaintext = aead.decrypt(raw[:_NONCE_BYTES], raw[_NONCE_BYTES:], _associated_data(task_id, generation, key_id))
        except InvalidTag as error:
            raise CheckpointCryptoError("checkpoint failed authentication (wrong key, changed bytes, or moved row)") from error
        return plaintext.decode("utf-8")

    def open_stored(self, task_id: str, generation: int, stored: str) -> str:
        ...
        return self.open(task_id, generation, stored)
