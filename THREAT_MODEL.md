...
- Host -> provider: a **canonical detached `ProviderView`** (Section 8 finding #2) and available tool names. Every provider — in-process or remote — is handed the same view (`task_id, goal, step, tool_calls, status, migrated, messages, tool_results`); the host's `memory` bookkeeping (`approvals`, `used_approval_ids`, `migration`), `checkpoint_generation`, and `result` are dropped. The remote adapters serialize a further-narrowed json wire projection of that view (`provider_state`: no `tool_results`, no `migrated` — tool output travels as projected `messages`). Controls: projection hides ungranted tool messages and narrows returned tool output fields; the view is frozen with read-only top-level containers and **deeply detached** into plain copies, so an in-process provider can neither read host bookkeeping nor mutate live host state through aliases. Evidence: `src/portmark/projection.py` (`provider_view`), `src/portmark/models.py` (`ProviderView`), `src/portmark/providers.py`.
